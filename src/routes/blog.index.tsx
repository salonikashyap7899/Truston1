...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Expert perspectives on Lucknow real estate, land investment, architecture, and building generational wealth.",
      },
    ],
  }),
...
function BlogIndexPage() {
  ...
  return (
    <div className="bg-[#04090f] text-white min-h-screen overflow-x-hidden selection:bg-[#00BFFF] selection:text-[#04090f]">
      ...
      {/* ── Featured Stories (only when "All" selected) ── */}
      {activeKey === "all" && (
      ...
      <section className="py-16 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          {filteredArticles.length === 0 ? (
            <div className="py-20 text-center text-white/30 text-sm">No articles in this category yet.</div>
          ) : (
              ...
              {filteredArticles.map((art, i) => (
                <Reveal key={art.slug} delay={i * 0.06}>
                  <Link
                    to="/blog/$slug"
                    params={{ slug: art.slug }}
                    className="block bg-[#060c16] cursor-pointer group hover:bg-[#080d1a] transition-colors duration-300 relative flex flex-col h-full"
                  >
          ...
          <Reveal>
            <div>
              ...
              <p className="text-white/45 text-sm mt-5 leading-relaxed font-light max-w-sm">
                Market intelligence, investment insights, and architectural inspiration — curated by TrustOn's experts.
              </p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
